...
const AnimatedView = native_1.animated(react_native_1.View);
class Passcode extends react_1.PureComponent {
    ...
    async shake() {
        ...
        await commons_1.delay(duration);
        this.setState({
    ...
    renderKeypadButton(text) {
        ...
        return (<AnimatedView style={this.fadeInAnimation.springs}>
        <react_native_1.TouchableHighlight underlayColor={this.props.keypadHighlightedColor} disabled={disabled} onShowUnderlay={() => this.setState({
          ...
          <react_native_1.View>
            <react_native_1.Text style={this.state.selectedButtonText === text
            ? [
            ...
            {this.props.alphabetCharsVisible && (<react_native_1.Text style={this.state.selectedButtonText === text
            ? [
    ...
    renderDeleteButton() {
        ...
        <react_native_1.View>
          {this.props.deleteButtonIcon ? (this.props.deleteButtonIcon) : (<>
              {!this.props.deleteButtonDisabled && (<native_1.animated.View key="delete" style={deleteAnimation.springs}>
                  <react_native_1.Image source={assets_1.Icons.ic_delete} resizeMode="cover"/>
                </native_1.animated.View>)}
              {this.props.deleteButtonText && (<react_native_1.Text>{this.props.deleteButtonText}</react_native_1.Text>)}
            </>)}
        </react_native_1.View>
    ...
    renderPasscodeIndicator() {
        ...
        return (<react_native_1.View style={styles.passcodeContainer}>
        {_.range(this.passcodeLength).map((value) => {
            var _a, _b, _c, _d, _e, _f, _g;
            const lengthSup = ((passcode.length >= value + 1 && !changeScreen) || showError) &&
                !attemptFailed;
            return (<>
              {((!this.props.passcodeVisible ||
                (!this.props.passcodeVisible && !lengthSup)) && (<react_native_1.View style={[
                styles.passcodeHiddenCharContainer,
                !_.isEmpty(this.state.passcode[value])
                    ? {
                        borderColor: this.state.showError
                            ? (_a = this.props.passcodeErrorColor) !== null && _a !== void 0 ? _a : commons_1.colors.fail : (_b = this.props.passcodeHighlightColor) !== null && _b !== void 0 ? _b : commons_1.colors.primary,
                    }
                    : {
                ...
                !_.isEmpty(this.state.passcode[value])
                    ? {
                        backgroundColor: this.state.showError
                            ? (_d = this.props.passcodeErrorColor) !== null && _d !== void 0 ? _d : commons_1.colors.fail : (_e = this.props.passcodeHighlightColor) !== null && _e !== void 0 ? _e : commons_1.colors.primary,
                    }
                    : {
                        backgroundColor: (_f = this.props.passcodeNormalColor) !== null && _f !== void 0 ? _f : commons_1.colors.transparent,
                    },
            ]}/>
                </react_native_1.View>)) || (<react_native_1.View style={[styles.passcodeTextCharContainer]}>
                  {this.state.passcode[value] && (<react_native_1.Text style={[
                ...
                this.props.passcodeHighlightColor
                  ...
                  {_.isEmpty(this.state.passcode[value]) && (<react_native_1.View style={[
    ...
    render() {
        ...
        <react_native_easy_grid_1.Grid style={styles.grid}>
          <react_native_easy_grid_1.Row key="firstRow" style={styles.row}>
            {_.range(1, 4).map((index) => {
            return (<react_native_easy_grid_1.Col key={index} style={styles.col}>
          ...
          <react_native_easy_grid_1.Row key="secondRow" style={styles.row}>
            {_.range(4, 7).map((index) => {
            return (<react_native_easy_grid_1.Col key={index} style={styles.col}>
                  {this.props.keypadButton
                ? this.props.keypadButton(index, this.handleKeypadTouch)
                : this.renderKeypadButton(index.toString())}
          ...
          <react_native_easy_grid_1.Row key="thirdRow" style={styles.row}>
            {_.range(7, 10).map((index) => {
            return (<react_native_easy_grid_1.Col key={index} style={styles.col}>
                  {this.props.keypadButton
                ? this.props.keypadButton(index, this.handleKeypadTouch)
                : this.renderKeypadButton(index.toString())}
          ...
          <react_native_easy_grid_1.Row key="lastRow" style={styles.row}>
            <react_native_easy_grid_1.Col key="bottomLeftButton" style={styles.col}>
              {this.props.bottomLeftButton}
            </react_native_easy_grid_1.Col>
            <react_native_easy_grid_1.Col key="0" style={styles.col}>
              {this.props.keypadButton
            ? this.props.keypadButton(0, this.handleKeypadTouch)
            : this.renderKeypadButton('0')}
            </react_native_easy_grid_1.Col>
            <react_native_easy_grid_1.Col key="delete" style={[styles.col]}>
              <>
